...
public class BusinessServiceImpl extends BaseServiceImpl<Business> implements BusinessService {
    ...
    public void reportExcel(List<Business> businessList, String savePath) {
        try {
            ...
            String titles = "序号,拿号日期,初评号,报备号,报告编号,项目名称,委托单位,评估对象,价估目的,估计方法,评估总值,约定金额,开票金额,到账金额,中介比例,实收金额,合作方,业务主办,主办工作量,业务助理,助理工作量,现场勘查,勘察工作量,初评日期,报告（拟写人）,盖章日期,出报告日期,报告份数,签字评估师,收费日期（日期空表示未收费）,收款方式,有无合同,否是已付中介费,底稿完成说明,注备1(基本),注备2(收费),录入人";
            String fields = "businessId,nhrq,preCode,baobeiId,baogaoId,xmmc,weituo,pgdx,gjmd,gjff,pgzz,ydje,kpje,dzje,zjbl,ssje,jjmc,ywzb,zbgzl,ywzl,zlgzl,xckc,kcgzl,cprq,bgr,gzrq,cbgrq,bgfs,qzpgs,sfrq,skfs,ywht,sfyfzjf,dgwcsm,beizhu1,beizhu2,username";
    ...
    public void reportMyExcel(List<Business> businessList, String savePath) {
        try {
            FileOutputStream out = new FileOutputStream(savePath);
            String titles = "序号,拿号日期,初评号,报告编号,项目名称,委托单位,评估对象,价估目的,估计方法,评估总值,约定金额,合作方,业务主办,主办工作量,业务助理,助理工作量,现场勘查,勘察工作量,初评日期,报告（拟写人）,盖章日期,出报告日期,报告份数,签字评估师,收费日期（日期空表示未收费）,有无合同,底稿完成说明,注备1(基本),录入人";
            String fields = "businessId,nhrq,preCode,baogaoId,xmmc,weituo,pgdx,gjmd,gjff,pgzz,dzje,jjmc,ywzb,zbgzl,ywzl,zlgzl,xckc,kcgzl,cprq,bgr,gzrq,cbgrq,bgfs,qzpgs,sfrq,ywht,dgwcsm,beizhu1,username";
            ExportUtil.ExportXls(businessList, out, fields, titles);
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    /**
     * 生成规则固定格式+报告数量
     * 穗和顺资字(Y)第0MI号 Y:2017 M:03 I:0001
     * @param year 拿号年
     * @param month 拿号月份
     * @return 新报告号
     */
