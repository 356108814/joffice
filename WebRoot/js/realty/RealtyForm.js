		...
		constructor : function(_cfg) {
			...
			RealtyForm.superclass.constructor.call(this, {
				...
				initComponents : function() {
						...
						items : [{
									...
									items : [ {
										...
										text : '上次报告编号:'
											...
											name : 'realty.preBaogaoId',
											width : 300,
											readOnly:true,
											xtype : 'textfield',
											id : 'realtyForm.preBaogaoId'
										}]
								},
								{
									xtype : 'fieldset',
					...
					this.loadData( {
						deferredRender : false,
						url : __ctxPath+ '/realty/getLastBaogaoIdRealty.do',
						root : 'data',
						success : function(response, options) {
							var res = Ext.util.JSON.decode(response.responseText).data;
							Ext.getCmp('realtyForm.preBaogaoId').setValue(res);
						},
						failure : function(response, options) {
							Ext.Msg.alert('Error',response.failureType+"=="+response.responseText);
						}
					...
					if (this.businessId == null || this.businessId == 'undefined') {
						...
						this.loadData( {
							deferredRender : false,
							url : __ctxPath+ '/realty/getNewBaogaoIdRealty.do',
							root : 'data',
							success : function(response, options) {
								var res = Ext.util.JSON.decode(response.responseText).data;
								Ext.getCmp('realtyForm.baogaoId').setValue(res);
								...
								Ext.getCmp('realtyForm.myid').setValue(year+indexS);
							},
							failure : function(response, options) {
								Ext.Msg.alert('Error',response.failureType+"=="+response.responseText);
							}
