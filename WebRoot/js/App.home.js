...
DiaryPanelView = Ext.extend(Ext.ux.Portlet, {
	...
	constructor : function(_cfg) {
		...
		DiaryPanelView.superclass.constructor.call(this, {
			...
			tools : this.tools,
			autoLoad:{
				url:__ctxPath+ '/system/displayDiary.do?start=0&limit=8',scripts:true
			}
		});
	},
	initTool : function() {
...
AppointmentPanelView = Ext.extend(Ext.ux.Portlet, {
	...
	initTool : function() {
		this.tools = [{
					id : 'refresh',
					handler : function() {
						Ext.getCmp('AppointmentPanelView').getUpdater().update(__ctxPath+ '/task/displayAppointment.do?start=0&limit=8');
					}
				}, {
					id : 'close',
					handler : function(e, target, panel) {
...
CalendarPlanPanelView = Ext.extend(Ext.ux.Portlet, {
	...
	initTool : function() {
		this.tools = [{
					id : 'refresh',
					handler : function() {
						Ext.getCmp('CalendarPlanPanelView').getUpdater().update(__ctxPath+ '/task/displayCalendarPlan.do?start=0&limit=8');
					}
				}, {
					id : 'close',
					handler : function(e, target, panel) {
...
MyPlanPanelView= Ext.extend(Ext.ux.Portlet, {
	...
	constructor : function(_cfg) {
		...
		MyPlanPanelView.superclass.constructor.call(this, {
			...
			tools : this.tools,
			autoLoad:{
				url:__ctxPath+ '/task/displayWorkPlan.do?start=0&limit=8',scripts:true
			}
		});
	},
	initTool : function() {
		this.tools = [{
					id : 'refresh',
					handler : function() {
						Ext.getCmp('MyPlanPanelView').getUpdater().update(__ctxPath+ '/task/displayWorkPlan.do?start=0&limit=8');
					}
				}, {
					id : 'close',
					handler : function(e, target, panel) {
		...
		this.tools = [
			{
					id : 'refresh',
					handler : function() {
						Ext.getCmp('DepPlanPanelView').getUpdater().update(__ctxPath+ '/task/displayDepWorkPlan.do');
					}
				},
				{
					id : 'close',
       ...
       this.buttons=[
       {
         xtype:'button',
       	...
       	handler:function(){
				...
				var portal = Ext.getCmp('Portal');
				var array=['DiaryPanelView','TaskPanelView','AppointmentPanelView','CalendarPlanPanelView','DepPlanPanelView','MyPlanPanelView','MyDocumentPanelView','MyMailPanelView'];
...
AppHome = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		if(confs==null|| confs==undefined||confs.length<1){
			...
			var p2={panelId:'CalendarPlanPanelView',column:0,row:1};
			var p3={panelId:'TaskPanelView',column:0,row:0};
			confs.push(p3);
			confs.push(p2);
			confs.push(p1);
		}
